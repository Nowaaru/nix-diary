...
let 
    ...
    useLanguage = withWhat: languageId:
       let
            languageTemplate = {
...
in ( lib.lists.foldl (acc: cur: acc // cur) {} [
    ...
    (useLanguage [ withLsp withFormatting withTreesitter (withDiagnostics ["eslint_d"]) (withOverrides { 
        lsp.server = "tsserver";
    ...
    (useLanguage [ withLsp withTreesitter (withOverrides {
        lsp.neodev.enable = true;
    }) ] "lua")

    (useLanguage [ withLsp withDebug withTreesitter (withOverrides {
        crates.enable = true;
    }) ] "rust")
])
